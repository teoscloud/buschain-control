...
//! 2. **FxParams** — Props on per-insert slots (knobs / mix / insert power)
//! 3. **FxRewire** — add/remove/reorder slots (spawn/stop one slot + exclusive links)
...
//! ## Hard rules
//!
//! - Per-insert FX slots: reorder/remove/add never rebuild unrelated slots.
...
//! - Apps stay on stable `buschain_*` null-sink buses; FX is per-slot `buschain_fx_*_*`.
...
pub enum LiveChange {
    ...
    /// Insert add / remove / reorder — slot spawn/stop + exclusive rewire.
