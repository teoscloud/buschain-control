...
- **Per-track device out (`Track.output_devices`):** a stem may send straight to a physical sink, **additive** to its Master / Track→Track sends (remove Master for a device-only stem). The hop is bridged through `buschain_rs_out_*` when the device rate differs from GraphClock, so every liveness probe on that path uses `egress_hop_live`, not a raw link check — including warm adopt, the wet-chain probe, and the muted-leak probe. Entries are sticky by description across unplug (`store::resolve_devices`, shown as `(offline)`); removing one destroys just that recorded bridge pair. Master keeps its single `session.master_output` HW out.
